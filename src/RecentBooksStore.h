...
class RecentBooksStore : public PersistableStore<RecentBooksStore> {
 ...
 public:
  ...
  // Update every catalog field learned by the idle metadata/cover worker while
  // preserving this store's recency order.
  void updateBookData(const RecentBook& updated);
