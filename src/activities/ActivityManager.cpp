...
void ActivityManager::goToShelves() {
  auto activity =
      makeUniqueNoThrow<CoverLibraryActivity>(renderer, mappedInput, CoverLibraryActivity::InitialView::SHELVES);
  if (!activity) {
    LOG_ERR("ACT", "OOM: shelves activity");
    return;
  }
  replaceActivity(std::move(activity));
}
