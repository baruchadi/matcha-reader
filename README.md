...
opens a paged Reading Hub cover grid, while **Show all shelves** remains a Reading Hub list. Queue shows the first five
books in their chosen order. Hold Confirm on a queued book to move it earlier or later. Read has one Recently completed
section, an explicit **Show all completed books** action, and up to six finished covers; the full collection is a paged
Reading Hub grid, and holding Confirm on any completed cover adds or edits its rating. These paths never drop into the
legacy cover-library screen; Back restores their parent Library or Read page.
...
of truth, so an absent or stale optional scan cache cannot reduce it to only recent titles. The native Library
reconciles its compact on-card index once per boot when first opened; that index includes EPUB, XTC, text, and manga
folders and is the sole structural source for shelves. The old cover-library cache is used only as validated optional
cover/series metadata. Only the visible six-book or seven-shelf page is resolved at a time, with no fixed shelf cap;
cover thumbnail paths are cached so focus movement does not rescan thumbnail directories. Missing visible thumbnails
and EPUB series metadata are generated by the shared cancellable cover worker after one idle second; any button press
cancels that background work immediately, so Reading Hub no longer depends on visiting the legacy Library UI.
